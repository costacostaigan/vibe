    ...
    <div data-testid="overview-page-wrapper">
      <div className="hidden small:block">
        <div className=" flex justify-between items-center mb-4">
          <h1 className="font-light" data-testid="welcome-message" data-value={customer?.first_name}>
            <Message name="modules.account.welcome"/>{' '}{customer?.first_name}
          </h1>
          <span className="text-small-regular text-ui-fg-base">
            <Message name="modules.account.signed_as"/>:{" "}
            <span className="font-semibold" data-testid="customer-email"
                  data-value={customer?.email}>{customer?.email}</span>
          </span>
        </div>
            ...
            <div className="flex items-start gap-x-16 mb-6">
              <div className="flex flex-col gap-y-4">
                ...
                <div className="flex items-end gap-x-2">
                  <span className="text-3xl-semi leading-none" data-testid="customer-profile-completion"
                        data-value={getProfileCompletion(customer)}>
                    {getProfileCompletion(customer)}%
                  </span>
                  <span className="uppercase text-base-regular text-ui-fg-subtle">
                    <Message name="modules.account.profile.completed"/>
                  </span>
                </div>
              </div>
              ...
              <div className="flex flex-col gap-y-4">
                ...
                <div className="flex items-end gap-x-2">
                  <span className="text-3xl-semi leading-none" data-testid="addresses-count"
                        data-value={customer?.shipping_addresses?.length || 0}>
                    {customer?.shipping_addresses?.length || 0}
                  </span>
                  <span className="uppercase text-base-regular text-ui-fg-subtle">
                    <Message name="modules.account.saved"/>
                  </span>
                </div>
              </div>
            </div>
                          ...
                          <Container className="bg-gray-50 flex justify-between items-center p-4">
                            <div className="grid grid-cols-3 grid-rows-2 text-small-regular gap-x-4 flex-1">
                              <span className="font-semibold">
                                <Message name="modules.account.orders.created_at"/>
                              </span>
                              <span className="font-semibold">
                                <Message name="modules.account.orders.number"/>
                              </span>
                              <span className="font-semibold">
                                <Message name="modules.account.orders.amount"/>
                              </span>
                            ...
                            <button className="flex items-center justify-between" data-testid="open-order-button">
                              <span className="sr-only">
                                <Message name="modules.account.order.go_to"/>{' '}#{order.display_id}
                              </span>
